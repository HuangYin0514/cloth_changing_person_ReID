...
def test_for_prcc(args, query_sc_loader, query_cc_loader, gallery_loader, model, use_gpu, ranks=[1, 5, 10], epoch=None):
    model.eval()
...
def test_for_ltcc(args, query_loader, gallery_loader, model, use_gpu, ranks=[1, 5, 10], epoch=None):
    ...
    cmc_2, mAP_2 = evaluate_ltcc(distmat, q_pids, g_pids, q_camids, g_camids, q_clothids, g_clothids, ltcc_cc_setting=True)
    if epoch:
        print("Epoch {}: ".format(epoch), end="")
    print("mAP: {:.4%}  ".format(mAP_2), end="")
    for r in ranks:
