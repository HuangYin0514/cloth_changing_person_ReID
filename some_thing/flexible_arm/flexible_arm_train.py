...
class MDANN(nn.Module):
    def __init__(self, in_dim=6, out_dim=3, hidden_dims=(128, 128, 128), dropout=0.0):
        super().__init__()
        layers = []
        prev = in_dim
...
class LNN(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(dof):
            grad_i = torch.autograd.grad(dLddq[:, i].sum(), x, create_graph=True)[0][:, dof:]
            M[:, i, :] = grad_i
...
def train_model(
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 20 == 0 or epoch == 1:
            ...
            print(f"  [{epoch:3d}/{epochs}] train={tr_loss:.6f}  " f"val={va_loss:.6f}  lr={lr_now:.2e}  " f"best_ep={history['best_epoch']}")
...
mdann, hist_mdann = train_model(mdann, Xtn, Ytn, Xvn, Yvn, model_name="MDANN", epochs=EPOCHS, lr=LR, batch_size=BATCH)

# ── LNN ──
lnn = LNN(dof=3, hidden_dims=HIDDEN)
lnn, hist_lnn = train_model(lnn, Xtn, Ytn, Xvn, Yvn, model_name="LNN", epochs=EPOCHS, lr=LR, batch_size=BATCH)

# ── HNN ──
hnn = HNN(dof=3, hidden_dims=HIDDEN)
hnn, hist_hnn = train_model(hnn, Xhtn, Yhtn, Xhvn, Yhvn, model_name="HNN", epochs=EPOCHS, lr=LR, batch_size=BATCH)
...
def eval_test_loss(model, X_te, Y_te, name):
    ...
    print(f"[{name}] 测试MSE={mse:.6f}  " f"RMSE_ddθ={np.degrees(rmse_theta):.4f}°/s²  " f"RMSE_ddq₁={rmse_q1*1e3:.4f}mm/s²  " f"RMSE_ddq₂={rmse_q2*1e3:.4f}mm/s²")
    return {"mse": mse, "rmse_theta": rmse_theta, "rmse_q1": rmse_q1, "rmse_q2": rmse_q2}
...
def savefig(name):
    plt.savefig(f"results/{name}", dpi=150, bbox_inches="tight", facecolor="#0d1117")
    plt.close()
...
fig.suptitle("训练损失曲线对比（归一化 MSE）", fontsize=13, fontweight="bold", color="#e6edf3")
for ax, (hist, name, col) in zip(
    ...
    ax.axvline(hist["best_epoch"], color="#f0883e", ls=":", lw=1.2, label=f'最佳 ep={hist["best_epoch"]}')
...
fig.suptitle("自回归预测 vs 真实轨迹（关节角度）", fontsize=13, fontweight="bold", color="#e6edf3")
...
# ── 图3：柔性模态预测对比 ──
fig, axes = plt.subplots(2, 2, figsize=(13, 9))
fig.suptitle("自回归预测 vs 真实轨迹（柔性模态）", fontsize=13, fontweight="bold", color="#e6edf3")
...
for mode_idx, row_axes in enumerate(axes):
    ...
    ax.set(xlabel="时间 [s]", ylabel="误差 [mm]", title=f"模态{mode_idx+1} 位移绝对误差")
    ax.legend(fontsize=9)
    ax.grid()
...
# ── 图4：能量误差对比（HNN 的优势）──
fig, axes = plt.subplots(1, 3, figsize=(15, 5))
fig.suptitle("预测轨迹能量误差对比（HNN 保辛结构优势）", fontsize=13, fontweight="bold", color="#e6edf3")
...
def compute_energy(pred):
    ...
    PE = np.array([0.5 * float(pred["q"][:, k] @ Kfc_np @ pred["q"][:, k]) for k in range(pred["q"].shape[1])])
    return KE + PE


true_E = compute_energy({"dtheta": test_traj["dtheta"][: ROLLOUT + 1], "q": test_traj["q"][:, : ROLLOUT + 1]})
...
ax = axes[1]
for E_pred, name, col in [(E_mdann, "MDANN", C[0]), (E_lnn, "LNN", C[1]), (E_hnn, "HNN", C[2])]:
...
x_pos = np.arange(3)
bars = ax.bar(x_pos, rmse_theta_all, color=[C[0], C[1], C[2]], width=0.5, edgecolor="#30363d")
for bar, val in zip(bars, rmse_theta_all):
    ax.text(bar.get_x() + bar.get_width() / 2, bar.get_height() + 0.01, f"{val:.2f}°", ha="center", va="bottom", fontsize=10, color="#c9d1d9")
ax.set_xticks(x_pos)
...
ax.set_title("三模型性能对比汇总", color="#e6edf3", fontsize=13, fontweight="bold", pad=20)
plt.tight_layout()
