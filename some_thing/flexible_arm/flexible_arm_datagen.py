...
def make_phi(beta_L):
    ...
    beta  = beta_L / L
    sigma = (np.cosh(beta_L) + np.cos(beta_L)) / \
            (np.sinh(beta_L) + np.sin(beta_L))
    phi   = lambda x, b=beta, s=sigma: (
        np.cosh(b*x) - np.cos(b*x) - s*(np.sinh(b*x) - np.sin(b*x)))
    dphi  = lambda x, b=beta, s=sigma: b*(
        np.sinh(b*x) + np.sin(b*x) - s*(np.cosh(b*x) - np.cos(b*x)))
    d2phi = lambda x, b=beta, s=sigma: b**2*(
        np.cosh(b*x) + np.cos(b*x) - s*(np.sinh(b*x) + np.sin(b*x)))
...
corF_pre = np.array([
    np.dot(gwts, rho * A * gpts * d1_gp[i]) for i in range(N_MODES)
])
...
def generate_trajectory(
    ...
    PE = np.array([0.5 * float(q[:, k] @ Kfc @ q[:, k])
...
def generate_dataset(
    ...
    for i in iter_range:
        ...
        if ttype == 'free':
            ...
            tau_fn   = make_torque('random_zoh', amp=amp,
                                   interval=interval, t_end=t_end)
    ...
    dataset = {
        ...
        'params': {
            'L': L, 'rho': rho, 'E': E, 'A': A, 'I_sec': I_sec,
            'J_hub': J_hub, 'd_damp': d_damp, 'g': g,
            'N_MODES': N_MODES, 'BETA_L': BETA_L,
            'Mfc': Mfc, 'Kfc': Kfc,
            'fn_theory': fn_theory,
            'theta_eq': -np.pi/2,
    ...
    print(f"  θ 均值={stats['theta_mean']:.3f} rad  "
          f"σ={stats['theta_std']:.3f}")
    print(f"  q₁均值={stats['q_mean'][0]*1e3:.4f}mm  "
          f"σ={stats['q_std'][0]*1e3:.4f}mm")
    return dataset
...
if __name__ == '__main__':
    ...
    train_data = generate_dataset(TRAIN_SIZE, 'train', seed=42,
                                  t_end=T_END, n_pts=N_PTS, noise_level=NOISE)
    val_data   = generate_dataset(VAL_SIZE,   'val',   seed=123,
                                  t_end=T_END, n_pts=N_PTS, noise_level=NOISE)
    test_data  = generate_dataset(TEST_SIZE,  'test',  seed=999,
                                  t_end=T_END, n_pts=N_PTS, noise_level=NOISE)
    ...
    X_seq, Y_seq = build_full_dataset(train_data, fmt='sequence',
                                      seq_len=20, stride=5)
