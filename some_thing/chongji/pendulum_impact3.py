...
def train_pendulum_pinn(params: PendulumParams, config: TrainingConfig, t0: float, verbose: bool = True):
    ...
    for epoch in range(config.adam_epochs):
        ...
        t0_region = torch.rand(200, 1) * 0.1 + (t0 - 0.05)
        t_left = torch.cat([t_left, t0_region[t0_region < t0]])
        t_right = torch.cat([t_right, t0_region[t0_region > t0]])
