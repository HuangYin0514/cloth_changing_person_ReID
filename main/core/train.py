...
def train(config, reid_net, train_loader, criterion, optimizer, scheduler, device, epoch, logger):
    ...
    for epoch, data in enumerate(tqdm(train_loader)):
        ...
        if config.MODEL.MODULE == "Lucky":
            ...
            black_feat = reid_net.black_pool(black_feat_map).view(B, reid_net.GLOBAL_DIM)
            black_bn_feat, black_cls_score = reid_net.black_classifier(black_feat)
            ...
            meter.update({"black_loss": black_loss.item()})
