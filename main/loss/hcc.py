...
"""
https://github.com/PKU-ICST-MIPL/DMA_TIFS2023/blob/main/loss.py
"""


def compute_dist_euc(x1, x2, p1, p2):
    m, n = x1.shape[0], x2.shape[0]
    dist = torch.pow(x1, 2).sum(dim=1, keepdim=True).expand(m, n) + torch.pow(x2, 2).sum(dim=1, keepdim=True).expand(n, m).t()
...
def compute_dist_kl(x1, x2, p1, p2):
    ...
    dist = (pr2 * x2.detach() - pr2 * x1.permute(1, 0, 2)).sum(dim=2) + (pr1 * x1.detach() - pr1 * x2.permute(1, 0, 2)).sum(dim=2).t()
